...
def get_question_by_id(question_id: int) -> dict[str, str | int] | None:
    df = load_questions()
    if df.empty or question_id not in set(df["id"].astype(int).tolist()):
        return None
    row = df.loc[df["id"].astype(int) == int(question_id)].iloc[0]
...
def render_question_image(
    ...
    """
    画像素材（動画用に横長）を生成します。
    旧デザイン（1080x1920想定）を、幅/高さに合わせて座標・フォントをスケールして再配置しています。
    """

    # 旧レイアウトの基準（縦長: 1080x1920）
    base_w, base_h = 1080, 1920
    width, height = size
    sx = width / base_w
    sy = height / base_h
    s_font = sy  # 縦方向が短いので、フォントは高さ基準で控えめに
    s_geom = min(sx, sy)

    base = Image.new("RGBA", size, (18, 11, 44, 255))
    draw = ImageDraw.Draw(base)

    # 背景の縦グラデ（旧実装をそのまま高さ方向にスケール）
    ...
    glow_draw.ellipse(
        (width - int(410 * sx), int(70 * sy), width - int(50 * sx), int(430 * sy)),
        fill=(255, 216, 151, 120),
    )
    ...
    draw.rounded_rectangle(
        card,
        radius=int(48 * s_geom),
        fill=(70, 42, 119, 218),
        outline=(233, 198, 126, 255),
        width=max(1, int(4 * s_geom)),
    )
    ...
    draw.rounded_rectangle(
        badge_box,
        radius=int(30 * s_geom),
        fill=(255, 246, 230, 240),
        outline=(239, 202, 127, 255),
        width=max(1, int(3 * s_geom)),
    )

    # フォント（旧サイズを sy でスケール）
    badge_font = _get_font(max(10, int(30 * s_font)))
    top_font = _get_font(max(12, int(44 * s_font)))
    ...
    footer_font = _get_font(max(10, int(26 * s_font)))
    ...
    # LOVE MESSAGE（旧: reply_to を表示していたので、意味が薄い「美咲の届いた恋のおたより」を削除）
    ...
    draw.rounded_rectangle(
        bubble,
        radius=int(38 * s_geom),
        fill=(252, 248, 255, 246),
        outline=(237, 201, 126, 255),
        width=max(1, int(3 * s_geom)),
    )

    max_text_width = (bubble[2] - bubble[0]) - int(110 * sx)
    lines = _wrap_text(draw, question, question_font, max_text_width)[:6]
    line_height = int(88 * sy)  # ←旧: 88
    line_height = max(18, line_height)
    ...
    # ここから下（「恋のおたより」〜フッター）を、ユーザー要望どおり削除
    # （旧: draw.text((148, 1525), "恋のおたより"...), タイムスタンプ/愛称など）

    # footer_font は残しているが今は描画しない
    _ = footer_font
...
def export_question_image(
    ...
    file_path = target_dir / f"question_{question_id:03d}_{safe_name}.png"
